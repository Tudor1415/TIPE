              ...
              begin
                if stop_array.(i) = 0 then (* Si la voiture ne s'est pas encore arrêté. et doit s'arrêter*)
                  begin
                    ...
                    let speed = 0 in
                    new_car_array.(i) <- V(tmps_opt, tmps_circ+dt, 0, vit_moy*n/(n+1) + speed/(n+1), stat_dep, stat_arr);
                  end
                else if stop_array.(i) = (-1) then (* Si la voiture doit repartir *)
                  begin
                    stop_array.(i) <- 0;
                    let speed = speed_control graph i car_array dt in
                    new_car_array.(i) <- V(tmps_opt, tmps_circ+dt, speed, vit_moy*n/(n+1) + speed/(n+1), stat_dep, stat_arr)
                  end
...
sim_terminate_on_exit_no_entry graph car_array_init dt stop_time 20;;
