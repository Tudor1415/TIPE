...
def trace_routes(data, minc = (-100,-100), maxc = (200,200), car_minc = (-53.57,3.24)) :
	route_map = np.zeros((int(np.floor(maxc[0]-minc[0])), int(np.floor(maxc[1]-minc[1]))))
	centered_data = []
	print("Starting route tracing.....")
	for ts in data :
		centered_ts = []
		for car in ts :
			centered_ts.append((car[0], (car[1][0]-minc[0]-car_minc[0], car[1][1]-minc[1]-car_minc[1])))
...
def draw_sectors(sector_map, x, y, sector_length, car=False) :
	for i in range(sector_length):
		for j in range(sector_length):
			if not(i == 0 or j == 0 or i == sector_length-1 or j == sector_length-1) :
				if car :
...
sector_length = 5
sectors, sector_map = get_sectors(rmap, sector_length, 5, 5)

color_sectors(list_data[30], sector_map, sectors, sector_length)
